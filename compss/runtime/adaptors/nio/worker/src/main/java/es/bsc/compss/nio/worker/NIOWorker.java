...
import es.bsc.compss.nio.commands.CommandTaskDone;
...
public class NIOWorker extends NIOAgent {
                ...
                try {
                    ...
                    if (param.isPreserveSourceData()) {
                        ...
                        if (!source.exists()){
                        	...
                        	//TODO The file to be copied needs to be serialized to a file from cache (or serialize from memory to memory
    ...
    public void sendTaskDone(NIOTask nt, boolean successful) {
        ...
        if (isWorkerDebugEnabled) {
        	WORKER_LOGGER.debug("Sending file " +workingDir + File.separator + "jobs" + File.separator + "job" + nt.getJobId() + "_" + nt.getHist() + ".out");
            c.sendDataFile(workingDir + File.separator + "jobs" + File.separator + "job" + nt.getJobId() + "_" + nt.getHist() + ".out");
        	WORKER_LOGGER.debug("Sending file " +workingDir + File.separator + "jobs" + File.separator + "job" + nt.getJobId() + "_" + nt.getHist() + ".err");
